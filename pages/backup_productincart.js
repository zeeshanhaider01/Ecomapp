...
export default function Cart(){
    ...
    const net_price=()=>{
        ...
        let sum=0
        ids.map((id)=>{sum+=data[id].price*cart[id]})
    ...
    if(Object.keys(cart).length!==0 && paymethod!=="" && /(^\d{14}$)/.test(CardNumber)==true && Date!=="" && /(^\d{3}$)/.test(Cvv)==true){
        ...
        console.log("checkoutbtn.current: ",checkoutbtn.current)
                ...
                <div>{ids.map((id)=>
                    <div style={{display:"flex"}}>
                        ...
                        <Button onClick={(e)=>decrement_handler(e,id)}>-</Button>
                            <Input className={styles.carttitle} onChange={({id,e})=>addToCarthandlertwo({id,e})} value={cart[id]}/>
                ...
                <Form>
                    ...
                    <form>
                        ...
                        <label>
                            ...
                            <label>
                            CVV:
                            <input type="number" onChange={e=>setCvv(e.target.value)} min="001" max="999" required/>
                            </label>
                        </label><br></br>  
